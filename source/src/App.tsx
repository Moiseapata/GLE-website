...
function Home() {
  ...
  const navItems = [['Notre action', 'action'], ['Vision', 'vision'], ['Programmes', 'programmes'], ['Sur le terrain', 'terrain'], ['Agir', 'don'], ['Journal', 'journal']];
  const visionObjectives = ['Reboisement & agroforesterie', 'Éducation au développement durable', 'Autonomisation des femmes et des jeunes', 'Diplomatie environnementale', 'Innovation en finance verte'];
  const submitContact = (values: ContactForm) => {
    ...
    contact.mutate({ data: values }, {
      ...
      onError: () => setContactSent('Votre message n’a pas pu partir. Réessayez dans un instant.'),
    });
  };
  const submitVolunteer = (values: VolunteerForm) => {
    ...
    volunteer.mutate({ data: { ...values, message: values.message || null } }, {
      ...
      onError: () => setVolunteerSent('Votre demande n’a pas pu partir. Réessayez dans un instant.'),
    });
  };
  const submitNewsletter = (values: NewsletterForm) => {
    ...
    newsletter.mutate({ data: values }, {
      ...
      onError: () => setNewsletterSent('L’inscription n’a pas pu être enregistrée. Réessayez dans un instant.'),
    });
  };
  ...
  return (
    <div className="page-shell noise">
      <header className="header-scrolled fixed top-0 z-40 w-full border-b border-[hsl(var(--border)/.65)]">
        <div className="section-wrap flex h-[72px] items-center justify-between">
          <button className="flex items-center gap-3 text-left" onClick={() => scrollToId('accueil')} data-testid="button-logo" aria-label="Revenir à l'accueil">
            ...
            <span className="leading-none"><span className="block font-bold tracking-[-.03em] text-[hsl(var(--primary))]">GREEN LEGACY INITIATIVE</span><span className="mt-1 block text-[.58rem] font-semibold uppercase tracking-[.16em] text-[hsl(var(--muted-foreground))]">{siteContent.brand.strapline}</span></span>
          </button>
      ...
      <main>
        <section id="accueil" className="relative overflow-hidden bg-[hsl(var(--primary))] pt-[72px] text-[hsl(var(--primary-foreground))]">
          ...
          <div className="section-wrap grid min-h-[680px] items-center gap-12 py-16 lg:grid-cols-[1.02fr_.98fr] lg:py-24">
            <div className="relative z-10"><div className="reveal mb-7 flex items-center gap-3 text-[hsl(var(--accent))]"><span className="h-px w-9 bg-[hsl(var(--accent))]" /><span className="eyebrow">{siteContent.hero.eyebrow}</span></div>
              ...
              {impact.isError && <p className="mt-4 text-xs text-[hsl(var(--accent))]" data-testid="status-impact-error">Les chiffres affichés sont nos dernières données publiées.</p>}
            </div>
        ...
        <section id="vision" className="section-pad bg-[hsl(var(--secondary))]"><div className="section-wrap grid gap-12 lg:grid-cols-[.75fr_1.25fr]"><div><p className="eyebrow mb-4">Notre vision</p><h2 className="display max-w-[420px] text-5xl leading-[.96] tracking-[-.05em] text-[hsl(var(--primary))] md:text-6xl">Un avenir<br /><span className="text-[hsl(var(--accent))]">durable et partagé.</span></h2><p className="mt-6 max-w-[380px] text-sm leading-relaxed text-[hsl(var(--muted-foreground))]">Bâtir un avenir durable, équitable et prospère pour les générations à venir.</p></div><div className="grid gap-4 sm:grid-cols-2"><p className="sm:col-span-2 text-sm font-bold uppercase tracking-[.12em] text-[hsl(var(--primary))]">Nos objectifs</p>{visionObjectives.map((item) => <div className="flex items-start gap-3 rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5" key={item} data-testid={`card-objective-${item}`}><span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[hsl(var(--accent)/.23)] text-[hsl(var(--primary))]"><Check size={15} /></span><p className="text-sm font-semibold leading-relaxed text-[hsl(var(--primary))]">{item}</p></div>)}</div></div></section>
